...
var RESULTS={empresario:[{min:0,max:3,label:"Patrimonio en Riesgo Crítico",color:"#C0392B",gauge:"#E74C3C",descFn:function(C){return'Estás dejando '+fmt(C.totalAnual)+'/año entre erosión, sobrecarga fiscal y oportunidad perdida. Con un patrimonio de '+fmt(C._pat)+', cada mes sin acción te cuesta '+fmt(Math.round(C.totalAnual/12))+'.'}},{min:4,max:7,label:"Estructura Deficiente",color:"#D4821A",gauge:"#E67E22",descFn:function(C){var fugas=[];if(C.erosion>0)fugas.push({n:'erosión',v:C.erosion});if(C.fiscal>0)fugas.push({n:'sobrecarga fiscal',v:C.fiscal});if(C.oport>0)fugas.push({n:'oportunidad',v:C.oport});if(C.impPat>0)fugas.push({n:'imp. patrimonio',v:C.impPat});fugas.sort(function(a,b){return b.v-a.v});var mayor=fugas.length?fugas[0]:null;return'Detectamos '+fugas.length+' fuga'+(fugas.length!==1?'s':'')+' en tu patrimonio de '+fmt(C._pat)+'.'+(mayor?' La mayor: '+fmt(mayor.v)+'/año en '+mayor.n+'.':'')+' Dinero que hoy sale sin que lo notes.'}},{min:8,max:11,label:"En Construcción",color:"#2471A3",gauge:"#3498DB",descFn:function(C){return'Buena base, pero aún hay '+fmt(C.totalAnual)+'/año en optimizaciones. Con ingresos de '+fmt(C._ingMes)+'/mes, eso es capital que podrías estar invirtiendo en vez de perdiendo.'}},{min:12,max:15,label:"Arquitectura Sólida",color:"#1E8449",gauge:"#27AE60",descFn:function(C){return'Buena estructura sobre '+fmt(C._pat)+'. Aún detectamos '+fmt(C.totalAnual)+'/año en ajustes finos — la diferencia entre bueno y excelente es esa cifra compuesta a 10 años.'}}],profesional:[{min:0,max:2,label:"Patrimonio en Riesgo Crítico",color:"#C0392B",gauge:"#E74C3C",descFn:function(C){var pctPas=C._gasMes>0?Math.round((C._pas/C._gasMes)*100):0;var mAut=C._gasMes>0?Math.round(C.liqBruta/C._gasMes*10)/10:0;return'Dependes 100% de tu ingreso activo de '+fmt(C._ingMes)+'/mes. Tus ingresos pasivos solo cubren el '+pctPas+'% de tus gastos. Si dejas de trabajar mañana, tu liquidez dura '+mAut+' meses.'}},{min:3,max:5,label:"Estructura Deficiente",color:"#D4821A",gauge:"#E67E22",descFn:function(C){var fugas=[];if(C.erosion>0)fugas.push({n:'erosión',v:C.erosion});if(C.fiscal>0)fugas.push({n:'sobrecarga fiscal',v:C.fiscal});if(C.oport>0)fugas.push({n:'oportunidad',v:C.oport});fugas.sort(function(a,b){return b.v-a.v});var mayor=fugas.length?fugas[0]:null;return'Detectamos '+fugas.length+' fuga'+(fugas.length!==1?'s':'')+' con erosión silenciosa de '+fmt(C.totalAnual)+'/año.'+(mayor?' La mayor: '+fmt(mayor.v)+'/año en '+mayor.n+'.':'')}},{min:6,max:7,label:"En Construcción",color:"#2471A3",gauge:"#3498DB",descFn:function(C){return'Buen camino, pero aún hay '+fmt(C.totalAnual)+'/año en oportunidades. Con ingresos de '+fmt(C._ingMes)+'/mes, esos ajustes marcan la diferencia.'}},{min:8,max:9,label:"Arquitectura Sólida",color:"#1E8449",gauge:"#27AE60",descFn:function(C){return'Buena estructura sobre '+fmt(C._pat)+'. Aún detectamos '+fmt(C.totalAnual)+'/año en optimización fina que compuesta a 10 años hace una gran diferencia.'}}]};
...
function showRes(nombre){
 ...
 document.getElementById('result-desc').textContent=lv.descFn?lv.descFn(C):(lv.desc||'');
 // Total loss
 ...
 // CTA
 document.getElementById('cta-amount').textContent=fmt(C.totalAnual)+'/año';
 document.getElementById('cta-sub').textContent='En 20 minutos revisamos tus fugas principales y te damos el plan concreto para cerrarlas. Sin rodeos.';
